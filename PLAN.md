...
> streamed half of it plus the last three Shubh-owned proposals (D1 request ids, D2 soft-budget
> alerts, D3 the 429/402 decision) landed 2026-08-02. The only Shubh item still on any list is
> Redis-backed reservations, which `PROPOSALS.md` A5 says is never in the 48-hour build.
...
*   **Tanay (Frontend & DX):**
    ...
    *   ✅ **DONE (Shubh, 2026-08-09).** "Model Efficiency" view on the dashboard, reading `model_efficiency`. The gap is decomposed into **rate** and **verbosity** rather than shown as one multiplier — "8x more expensive" conflates a vendor decision with a prompt one. The table is written offline by `scripts/cross_model.py --push`, never from the request path (`PROPOSALS.md` B11). ⚠ **The live comparison has not been run**: it needs a funded Anthropic key and spends real money, so the card renders its empty state until someone runs it.
...
*   ✅ **DONE — Shubh (Proxy & Infra):** ~~Stress test the proxy. Fix any race conditions in the DB writes from concurrent requests.~~
    ...
    *   ✅ **Sustained-load soak built and passing** (`tests/load_soak.py`, 2026-08-01; streamed mode 2026-08-02). N clients drive the enforced path while the Treasurer writes to the same `meter.db`. At 16 clients / 20s with ceilings enforced: **~7,500 requests at ~372 req/s (35% of a 1,067 req/s no-proxy baseline the harness measures itself), every one ledgered, zero `database is locked`, zero failed ledger writes, worst event-loop stall 24ms, every reservation released.** CLAUDE.md's two-writer claim now has evidence rather than an argument behind it. The throughput ceiling is the A5 design (one SQLite connection behind a lock), not a defect. **`--stream` covers the streamed path**, with responses deliberately outliving their reservation TTL, and `--break-heartbeat` proves that check can actually fail.
    *   ⚠ **The first three versions of this harness measured the wrong thing, which is worth knowing before trusting any load number here.** It hammered the *real* OpenAI API instead of the fake upstream (env override applied after `proxy.config` had frozen its values); the fake upstream answered 422 to every call; and the `meter.yaml` used a misspelled key, so **no ceilings loaded and the "enforced path" was never enforced**. All three now have explicit guards that refuse to run rather than silently measuring nothing.
