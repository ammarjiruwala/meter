...
async def lifespan(app: FastAPI):
    ...
    # file path, which survived the Postgres port and made the first line of every boot
    # announce a `meter.db` that does not exist, directly contradicting the line above it.
    # The host is worth printing: pointing at the wrong database is the failure that looks
    # like "where did all the data go".
    log.info("ledger ready at %s schema=%s (%d meter key(s) seeded)",
             (config.DATABASE_URL or "").split("@")[-1].split("?")[0] or "unset",
             config.DB_SCHEMA, seeded)
