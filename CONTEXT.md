...
*   **Predictive Engine: WORKING (v3), WIRED INTO THE PROXY, SELF-TUNING OFFLINE.** `predictor/` — method in `predictor/DESIGN.md`, contract in `predictor/README.md`, self-check `python tests/test_predictor.py` (122 checks).
    ...
    *   **Online loop is built, gated, and currently FLAT — not an improvement.** `predictor/refresh.py` reads the ledger, fits on the older 75%, scores against the newest 25%, and installs only if held-out median improves. Worst case is "no change". `scripts/prequential.py` measures whether it learns: first third 55.1%, last third 63.9%, and a permutation test puts a swing that size at 18% under random reordering — batch noise, not learning. Expected: WildChat keys group unrelated strangers' prompts with no shared template.
    ...
    *   **Open question, the one that matters:** does per-`(project, feature)` history work on genuinely *templated* traffic? WildChat structurally cannot answer it. The test is ~200 real calls across 5 templates × 40 slot variations, ~$0.05.
    *   **Data:** `data/wildchat/{train,validation,test}.jsonl` (522/150/75, committed — regenerating means re-paging a rate-limited API). `data/calibration/*.jsonl` — 45 real API observations. **`test.jsonl` is the locked set: score it once, at the end.**
