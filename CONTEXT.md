...
*   **Predictive Engine: WORKING (v3), WIRED INTO THE PROXY, SELF-TUNING OFFLINE.** `predictor/` — method in `predictor/DESIGN.md`, contract in `predictor/README.md`, self-check `python tests/test_predictor.py` (122 checks).
    ...
    *   **Online loop is built and gated.** `predictor/refresh.py` reads the ledger, fits on the older 75%, scores against the newest 25%, and installs only if held-out median improves. Worst case is "no change", which is what makes it safe to run unattended.
