...
\*   Feedback Loop: Compare \`predicted\_output\_tokens\` vs \`actual\_output\_tokens\`. Save variance to DB to calculate predictor accuracy.

\#\#\# B. The Treasurer Agent (Prava Integration)  
\*   An \`asyncio\` background loop that checks the \`wallets\` table every few seconds.  
\*   If \`provider\_balance \< threshold\`, calculate top-up amount (shortfall \+ buffer).  
