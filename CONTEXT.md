...
*   **Open blockers/decisions:**
    ...
    3.  **🚨 The Anthropic account has a $0 credit balance — this blocks a MUST BUILD item.** The key authenticates fine, but every call returns `"Your credit balance is too low to access the Anthropic API"`. §3 lists REAL LLM CALLS as MUST BUILD *because* the predictive engine needs real token usage to calibrate against; with no credits, Ammar's predicted-vs-actual variance and the whole cross-model efficiency comparison (PLAN.md Phase 3) have no data. Needs credits on the account or a funded key. Also **rotate the current key** — it was shared over chat. (`PROPOSALS.md` C4)
