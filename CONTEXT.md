...
*   **Open blockers/decisions:**
    ...
    3.  **No funded OpenAI key.** Anthropic is now working end to end against a real funded key — real completions, real streams, real usage, priced correctly. `OPENAI_API_KEY` is still the placeholder, so the GPT-4o half of the cross-model efficiency comparison (PLAN.md Phase 3) and `predictor/calibrate.py`'s real-ratio measurement have no provider. §3 lists REAL LLM CALLS as MUST BUILD. Needs a funded OpenAI key. Also **rotate the Anthropic keys** — both were shared over chat. (`PROPOSALS.md` C4)
