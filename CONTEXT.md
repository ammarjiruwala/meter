...
*   **Dashboard: LAYOUT + LIVE LOGS WORKING, RESTYLED.** `dashboard/` — Next.js (App Router) +
    ...
    *   Not yet done: **Model Efficiency view** (Phase 3, needs Ammar's cross-model data — B11).
        `model_efficiency` still appears only in `PLAN.md` and a comment in `treasury/db.py`;
        there is no table and no cross-model suite, so this is the dashboard's one remaining
        blocked item. (The Agent Activity panel that used to sit on this line is **done** — see
        the Treasurer Agent entry above.)
