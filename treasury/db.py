...
# contention — and FastAPI genuinely does that, running sync routes in a threadpool
# while async ones run on the event loop.
#
# Postgres removes the hazard rather than mitigating it: the pool hands every caller its
# own connection. Kept as a null context so the `with _lock:` blocks below stay
# untouched — see `pg._Connection` for why the call sites were left alone.
_lock = contextlib.nullcontext()


def _fetchone(sql: str, params: tuple = ()) -> dict[str, Any] | None:
    connect()
    return pg.fetchone(sql, params)


def _fetchall(sql: str, params: tuple = ()) -> list[dict[str, Any]]:
    connect()
    return pg.fetchall(sql, params)
...
CREATE TABLE IF NOT EXISTS mandates (
    id                   TEXT PRIMARY KEY,
    provider             TEXT NOT NULL,
    max_per_txn_usd      double precision NOT NULL,
    max_daily_usd        double precision NOT NULL,
...
def connect() -> pg._Connection:
    """Ensure the treasury tables exist, and hand back the pooled connection facade.

    The PRAGMAs are gone with SQLite. `busy_timeout` in particular existed because the
    proxy and the Treasurer were two writers against one file; Postgres's MVCC means
    they no longer contend at all.
    """
...
def _migrate(conn: pg._Connection) -> None:
    ...
    hit "no such column" rather than anything self-explanatory. ``ADD COLUMN`` is cheap
    and this stays correct once the tables are created fresh.
    """
    for column, ddl in (
        ("recurring_frequency", "TEXT"),
        ("status", "TEXT"),
        ...
        if not pg.column_exists("mandates", column):
            conn.execute(f"ALTER TABLE mandates ADD COLUMN IF NOT EXISTS {column} {ddl}")
